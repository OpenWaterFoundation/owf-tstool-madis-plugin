...
#   ./tstool/NN/plugins/owf-tstool-madis-plugin/
# - output is to the plugin 'dist' folder with filename like:
#     tstool-madis-plugin-1.0.0-win-2206060102.zip
...
createPluginZipFile() {
  ...
  "${sevenzip}" a -tzip ${zipFile} owf-tstool-madis-plugin
  exitStatus=$?
...
now=$(date +%Y%m%d%H%M)
zipFile="${distFolder}/tstool-madis-plugin-${pluginVersion}-win-${now}.zip"
