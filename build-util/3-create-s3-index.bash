#!/bin/bash
#
# Create the TSTool MADIS plugin product 'index.html' page on Amazon S3,
...
#
# software.openwaterfoundation.org/tstool-madis-plugin/index.html
# software.openwaterfoundation.org/tstool-madis-plugin/index.csv
#
...
#   2018-12-04 16:16:22          0 tstool-madis-plugin/
#   2018-12-04 16:16:37          0 tstool-madis-plugin/1.0.0/
#   2018-12-04 16:17:19   46281975 tstool-madis-plugin/1.0.0/tstool-madis-plugin-win-1.0.0-2201020304.zip
createCatalogFile() {
  ...
  if [ ${errorCode} -ne 0 ]; then
    logError ""
    logError "Error listing TSTool MADIS plugin download files to create catalog (maybe missing --aws-profile)?"
    exit 1
  fi
  ...
  tmpS3CatalogPath="/tmp/${USER}-tstool-madis-plugin-catalog-ls-installers.txt"
  # Only list installers that match the current download filename specification
  cat ${tmpS3ListingPath} | grep -E 'tstool-madis-plugin.*.zip' > ${tmpS3CatalogPath}
  logInfo "Available TSTool MADIS plugin installers:"
...
<p>
The TSTool MADIS Plugin software is available for Windows.
...
<ol>
<li> Download a TSTool MADIS Plugin Windows installer (zip file) from below.</li>
<li> Follow the latest <a href="https://software.openwaterfoundation.org/tstool-madis-plugin/latest/doc-user/appendix-install/install/">installation instructions</a>
...
<p>Linux versions of the TSTool MADIS Plugin are not actively developed.
    ...
    cat "${tmpS3CatalogPath}" | grep "${downloadOs}-" | awk '{ printf "%s %s %s %s\n", $1, $2, $3, $4 }' | sed -E 's|([0-9][0-9]/)|\1-zzz|g' | sed 's|/-zzz|-zzz|g' | sed 's|dev|-dev|g' | sort -r -k4,4 | sed 's|-zzz||g' | sed 's|-dev|dev|g'  | awk '
      {
        ...
        downloadFileProduct=""            # The product name (e.g., TSTool MADIS Plugin)
        ...
        if ( filenameSpecVersion = 1 ) {
          # Initial filename spec using format:  tstool-madis-plugin/1.0.0.dev/software/tstool-madis-plugin-1.0.0.dev-win-2201020304.zip
          ...
          downloadFileProduct="tstool-madis-plugin"
          downloadFileVersion=downloadFileParts[4]
          downloadFileOs=downloadFileParts[5]
        }
        # Set the URL to the download file.
        downloadFileUrl=sprintf("https://software.openwaterfoundation.org/tstool-madis-plugin/%s/software/%s", downloadFileVersion, downloadFile)
        product = "TSTool MADIS Plugin"
        ...
        docDevUrl=sprintf("https://software.openwaterfoundation.org/tstool-madis-plugin/%s/doc-dev/",downloadFileVersion)
        ...
        docUserUrl=sprintf("https://software.openwaterfoundation.org/tstool-madis-plugin/%s/doc-user/",downloadFileVersion)
...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Create the software.openwaterfoundation.org/tstool-madis-plugin/index.html and related files."
...
printVersion() {
  ...
  echoStderr ""
  echoStderr "TSTool MADIS Plugin"
...
uploadIndexFiles() {
  ...
  logInfo "Assuming that TSTool MADIS Plugin installer files were previously uploaded to S3."
  ...
  if [ -f "${s3CatalogTxtFileUrl}" ]; then
    ...
    if [ ${errorCode} -ne 0 ]; then
      logError ""
      logError "Error uploading TSTool MADIS Plugin catalog file."
      # Not fatal.
    fi
  else
  ...
  #${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-madis-plugin/index.html*" "/tstool-madis-plugin/" "/tstool-madis-plugin" --profile "${awsProfile}"
  #${awsExe} cloudfront create-invalidation --distribution-id ${cloudFrontDistributionId} --paths "/tstool-madis-plugin/index.html*" --profile "${awsProfile}"
  if [ "${operatingSystem}" = "mingw" ]; then
...
s3FolderUrl="s3://software.openwaterfoundation.org/tstool-madis-plugin"
gpDownloadUrl="https://software.openwaterfoundation.org/tstool-madis-plugin"
...
user=""
tmpS3ListingPath="/tmp/${USER}-tstool-madis-plugin-catalog-ls.txt"
